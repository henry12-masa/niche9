...
function renderHome(){
  ...
  window.examList.forEach(exam => {
    ...
    btn.innerHTML = `<h3>${exam.title}</h3><p>${exam.description}</p><p class="muted">20問収録 / ランダム50問モード対応</p>`;
...
function startQuiz(slug){
  ...
  if(!currentExam || bank.length === 0){ alert('問題データが見つかりません。'); return; }

  // 各資格20問の場合でも50問出題できるよう、重複なしで全資格から補充する
  const own = shuffle(bank);
  const other = shuffle(Object.entries(window.quizData)
    .filter(([key]) => key !== slug)
    .flatMap(([, questions]) => questions));

  currentQuestions = shuffle([...own, ...other]).slice(0, QUESTION_COUNT);
  currentIndex = 0;
  score = 0;
  $('examTitle').textContent = currentExam.title;
  $('scoreText').textContent = '正解 0';
  history.replaceState(null, '', `?exam=${encodeURIComponent(slug)}`);
  showScreen('quiz');
  renderQuestion();
}
...
renderHome();
const initialExam = getParam('exam') || getParam('type');
if(initialExam && window.examList.some(e => e.slug === initialExam)) startQuiz(initialExam);
